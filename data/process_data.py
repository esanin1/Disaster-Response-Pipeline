...
def load_data(messages_filepath, categories_filepath):
    ...
    :param categories_filepath: filepath to the  categories.csv categories file
    :return: pandas dataframe of joint messages and categories data
    '''
...
def clean_data(df):
    '''
    :param df: pandas dataframe to clean and transform
    :return: cleaned pandas dataframe for ML processing
    '''
    ...
    # drop rows where "related" category is not in [0, 1]
    df = df.drop(df[~df['related'].isin(['0', '1'])].index)
    return (df)

def save_data(df, database_filepath, table_name):
    '''
    :param df: paddas df to save in SQL db
    :param database_filepath: filepath to SQL db
    :param database_tablename: table name to save df to
    :return: None

    Note: Version 2.0.0 (January 26, 2023) of SQLAlchemy is not compatible with earlier versions of pandas.
